...
export default function ResetPassword() {
  ...
  const key = router.query.access_token;

  const dispatch = useDispatch();

  const passwordSchema = new yup.ObjectSchema({
  ...
  async function formSubmit(form) {
    ...
    dispatch(
      authActions.resetPassword({
        ...pswChangeForm,
        onSuccess: () => router.push('/reset-password-successfully')
      })
    );
  }
  ...
  useEffect(() => {
    if (error) {
      setError('email', { type: 'manuel', message: 'An error occurred' });
    }
  }, [error, setError]);
  useEffect(
    () => () => {
          ...
          <div className="flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
            <div className="mx-auto w-full max-w-lg lg:w-[360px]">
              <div className="text-center">
                ...
                <h1 className="text-slate-800 mb-3 text-3xl font-semibold">Reset password</h1>
              ...
              <div className="mt-8">
                <div className="mt-6">
                  <form onSubmit={handleSubmit(formSubmit)} className="space-y-6 mb-8">
                    <Input
                      label="New Password"
                      id="password"
                      type="password"
